...
### 課題C：Googleフォーム URL の設定（未完了）
- お問い合わせセクションに「入会・お問い合わせフォーム」ボタンを設置済み
- ただしリンク先は `GOOGLE_FORM_URL` というプレースホルダーのまま
- ユーザーが Google フォームを作成 → URL を取得 → index.html 内の
  `GOOGLE_FORM_URL`（1か所）を実URLに置換する必要がある
- フォームの項目案は「フォーム作成ガイド.md」に記載済み
- メールリンク（mailto:surugadai.nishi@gmail.com）は設定済み・変更不要

### お知らせデータの現状（2025年5-7月号を削除済み）
- notices.json は VOL.38 / 37 / 36 / 35 の4号。すべてPDF付き
- VOL.34（2025年5-7月号）はユーザー判断で削除済み
